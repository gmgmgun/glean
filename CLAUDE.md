...
| YouTube 자막 | `youtube-transcript` | API 키 불필요, oEmbed로 제목·저자 함께 가져옴. 음성 전사(Whisper)는 자막 없는 영상이 누적되면 검토 |

### 의도적으로 단순하게 둔 부분

- **본문 추출**: HTML은 정규식으로 태그 제거. YouTube는 자막 추출 분기. 향후 `@mozilla/readability` 등으로 업그레이드 여지.
...
4. **요약 형식**: 5~7문장. 핵심 내용 + SW 엔지니어가 생각해볼만한 포인트(HW가 SW에 미치는 영향, 디버깅·최적화 단서, 빠지기 쉬운 함정, 추가로 파볼 가지 등) 1~2개. 추측·일반론 금지, 본문 근거 있을 때만.
