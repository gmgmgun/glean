...
1. **summary**: 5~7문장의 한국어 요약. 다음을 자연스럽게 포함:
   - 핵심 내용 (이 자료가 무엇을 다루는지)
   - **SW 엔지니어가 생각해볼만한 포인트 1~2개** — 단순한 "왜 알 가치가 있는지" 보다, HW 동작이 SW 성능·설계·디버깅에 어떻게 영향을 주는지, 관련 시스템 만들 때 고려해야 할 것, 자주 빠지는 함정, 추가로 파볼 만한 가지(branch) 등 구체적 사고 단서.
   - 추측이나 일반론으로 채우지 말 것. 본문에 근거가 있을 때만 포인트 제시. 모호하면 핵심 내용 위주로 6문장.
...
const SCHEMA = {
  ...
  properties: {
    summary: {
      type: 'string',
      description: '5~7문장 한국어 요약. 핵심 내용 + SW 엔지니어가 생각해볼 포인트.',
    },
    tags: {
      type: 'array',
      items: { type: 'string' },
